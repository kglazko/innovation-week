...
class BrowserFragment : IWebViewLifecycleFragment() {
    ...
    fun onToolbarEvent(event: ToolbarEvent, value: String?) {
        val context = context!!
        when (event) {
            BACK -> if (webView?.canGoBack() ?: false) webView?.goBack()
            FORWARD -> if (webView?.canGoForward() ?: false) webView?.goForward()
            TURBO -> {
                when (value) {
            ...
            SETTINGS -> Unit // No Settings in BrowserFragment
            CAT -> webView?.loadUrl("https://kglazko.github.com/innovation-week")
            PIN_ACTION -> this@BrowserFragment.url?.let { url -> onPinToolbarEvent(context, url, value) }
            HOME -> if (!fragmentManager.getNavigationOverlay().isVisibleAndNonNull) {
                callbacks?.setNavigationOverlayIsVisible(true)
            }

            LOAD_URL -> throw IllegalStateException("Expected $event to be handled sooner")
        }
        Unit
    }
